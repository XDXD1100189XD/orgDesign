...
interface BulkState { col: string; value: string; }
...
function inferType(rows: GenRow[], key: string): 'string' | 'number' | 'boolean' {
...
function mergeExcelWithDashboard(
  ...
  const byEmpId = new Map<string, { depth: number; span: number; open_role: boolean; manager: string }>();
  const byName  = new Map<string, { depth: number; span: number; open_role: boolean; manager: string }>();
  ...
  const ID_KEYS   = ['employee_id','emp_id','id','worker_id','staff_id','person_id'];
  const NAME_KEYS = ['name','employee_name','full_name','display_name','worker_name','employee'];
  ...
  return excelRows.map((row, i) => {
    ...
    const match = (idVal && byEmpId.get(idVal))
               || (nameVal && byName.get(nameVal));
...
export default function TableView({ data, file }: Props) {
  ...
  const [sort,         setSort]         = useState<{ key: string; dir: SortDir }>({ key: '', dir: 'asc' });
  ...
  const defMap     = useMemo(() => Object.fromEntries(colDefs.map(d => [d.key, d])), [colDefs]);
  ...
  const filteredRows = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      r = r.filter(row => Object.values(row).some(v => cellStr(v).toLowerCase().includes(q)));
    }
  ...
  const validMap = useMemo(() => {
    const idKey   = colDefs.find(c => /^(emp_?id|employee_?id|id)$/i.test(c.key))?.key;
    const nameKey = colDefs.find(c => /^(name|display_?name|employee_?name|full_?name)$/i.test(c.key))?.key;
    const roleKey = colDefs.find(c => /^(role|title|job_?title|position)$/i.test(c.key))?.key;
  ...
  function toggleColVisible(key: string) {
    setColDefs(prev => prev.map(d => d.key === key ? { ...d, visible: !d.visible } : d));
  }
  ...
  function toggleSelectPage() {
    ...
    setSelected(prev => {
      const n = new Set(prev);
      pagedRows.forEach(r => allSel ? n.delete(cellStr(r._id)) : n.add(cellStr(r._id)));
      return n;
    });
  }

  // column drag-reorder
  function onThDragStart(key: string) { wasDragging.current = false; dragKey.current = key; }
  function onThDrag()                  { wasDragging.current = true; }
  ...
  const allPageSel     = pagedRows.length > 0 && pagedRows.every(r => selected.has(cellStr(r._id)));
    ...
    <div className={styles.root}>
      ...
      <div className={styles.toolbar}>
        ...
        <div className={styles.searchWrap}>
          <svg className={styles.searchIcon} viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.8">
            <circle cx="8.5" cy="8.5" r="5.5"/><path d="m13.5 13.5 3.5 3.5"/>
          </svg>
          <input
        ...
        <div className={styles.toolbarRight}>
          ...
          >
            ...
            Filters
            {activeFilters > 0 && <em className={styles.chip}>{activeFilters}</em>}
          </button>

          {/* Columns */}
          <div className={styles.colMenuWrap} ref={colMenuRef}>
            <button className={styles.tbBtn} onClick={() => setShowColMenu(s => !s)}>
              <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14">
              ...
              Columns
              <em className={styles.chip}>{colDefs.filter(c => c.visible).length}/{colDefs.length}</em>
            </button>
            {showColMenu && (
              <div className={styles.colMenu}>
                  ...
                  {colOrder.map(key => {
                    ...
                    return (
                      <label key={key} className={styles.colMenuRow}>
                        <input type="checkbox" checked={col.visible} onChange={() => toggleColVisible(key)} />
                        <span>{col.label}</span>
                      </label>
                    );
                  })}
                ...
                <div className={styles.colMenuFoot}>
                  <button onClick={() => setColDefs(d => d.map(c => ({ ...c, visible: true })))}>Show all</button>
                  <button onClick={() => setColDefs(d => d.map((c, i) => ({ ...c, visible: i < 6 })))}>Hide extras</button>
                </div>
              </div>
            )}
          </div>
          ...
          {/* Undo */}
          <button className={styles.tbBtn} onClick={undo} disabled={!undoStack.length} title="Undo">
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14">
              <path d="M4 7H13a4 4 0 0 1 0 8H7"/><path d="M7 4 4 7l3 3"/>
            </svg>
            Undo
          </button>

          {/* Reset */}
          <button className={`${styles.tbBtn} ${styles.tbBtnDanger}`} onClick={reset} title="Reset all edits">
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14">
              <path d="M3.5 9A7 7 0 1 1 5 14.66"/><path d="M3 5v4h4"/>
            </svg>
            Reset
          </button>

          {/* Export */}
          <button className={`${styles.tbBtn} ${styles.tbBtnPrimary}`} onClick={() => downloadCSV(filteredRows, colDefs)}>
            <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" width="14" height="14">
        ...
        <div className={styles.filterPanel}>
          <div className={styles.filterPanelInner}>
            {filterableCols.length === 0 ? (
              <span className={styles.filterEmpty}>No categorical columns detected for filtering</span>
              ...
              <div key={col.key} className={styles.filterItem}>
                ...
                >
                  ...
                  {filterOpts[col.key].map(v => (
                    <option key={v} value={v}>{v || '(blank)'}</option>
                  ))}
                </select>
              </div>
            ))}
          ...
          {activeFilters > 0 && (
            <button className={styles.clearFiltersBtn} onClick={() => setColFilters({})}>
      ...
      <div className={styles.statusBar}>
        <div className={styles.statusLeft}>
          <span className={styles.rowCount}>
            <b>{filteredRows.length.toLocaleString()}</b> / {rows.length.toLocaleString()} rows
          </span>
          {selected.size > 0 && (
          ...
          {undoStack.length > 0 && (
            <span className={styles.editedBadge}>● {undoStack.length} unsaved edit{undoStack.length > 1 ? 's' : ''}</span>
          )}
        </div>
        <div className={styles.statusRight}>
          {selected.size > 0 && (
            <>
              <button
                ...
                onClick={() => { setBulk({ col: orderedCols[0]?.key ?? '', value: '' }); setBulkMode('selected'); }}
              >
          ...
          <button
            className={`${styles.tbBtn} ${styles.tbBtnSmall}`}
            onClick={() => { setBulk({ col: orderedCols[0]?.key ?? '', value: '' }); setBulkMode('filtered'); }}
          >
      ...
      {bulk && (
        <div className={styles.bulkPanel}>
          <span className={styles.bulkLabel}>
            Set for <b>{bulkMode === 'selected' ? `${selected.size} selected` : `all ${filteredRows.length.toLocaleString()} filtered`}</b> rows:
          </span>
          <select
            className={styles.bulkSelect}
            value={bulk.col}
            onChange={e => setBulk(b => b ? { ...b, col: e.target.value } : null)}
          >
            {orderedCols.map(c => <option key={c.key} value={c.key}>{c.label}</option>)}
          </select>
          <span className={styles.bulkTo}>→</span>
          <input
            className={styles.bulkInput}
            value={bulk.value}
            onChange={e => setBulk(b => b ? { ...b, value: e.target.value } : null)}
            placeholder="New value…"
            onKeyDown={e => e.key === 'Enter' && applyBulk(bulkMode)}
            autoFocus
          />
          <button className={styles.bulkApply} onClick={() => applyBulk(bulkMode)}>Apply</button>
          <button className={styles.bulkCancel} onClick={() => setBulk(null)}>Cancel</button>
        </div>
      )}
      ...
      <div className={styles.tableWrap}>
        <table className={styles.table}>
          <thead>
            <tr>
              <th className={styles.checkTh}>
                <input type="checkbox" checked={allPageSel} onChange={toggleSelectPage} />
              </th>
              <th className={styles.flagTh} title="Row validation status">⚑</th>
              {orderedCols.map(col => (
                <th
                  key={col.key}
                ...
                >
                  <span className={styles.thGrip} title="Drag to reorder">⠿</span>
          ...
          <tbody>
            {pagedRows.length === 0 ? (
              <tr>
                <td colSpan={orderedCols.length + 2} className={styles.emptyRow}>
              ...
              return (
                ...
                >
                  ...
                  <td className={styles.flagTd}>
                    {flags
                      ? <span className={styles.flagWarn} title={flags.join('\n')}>⚠</span>
                      : <span className={styles.flagOk}>✓</span>}
                  </td>
                  {orderedCols.map(col => {
                    const isEditing = editCell?.rowId === rowId && editCell?.col === col.key;
                    ...
                    return (
                      <td
                        ...
                        onClick={() => {
                          if (!isEditing) { setEditCell({ rowId, col: col.key }); setEditValue(display); }
                        }}
                      >
                        {isEditing ? (
                          <input
      ...
      {totalPages > 1 && (
        <div className={styles.pagination}>
          <button className={styles.pgBtn} disabled={page === 1} onClick={() => setPage(1)}>«</button>
          ...
          <button className={styles.pgBtn} disabled={page === totalPages} onClick={() => setPage(totalPages)}>»</button>
          <span className={styles.pgInfo}>
            Page {page} of {totalPages} · rows {((page - 1) * PAGE_SIZE) + 1}–{Math.min(page * PAGE_SIZE, filteredRows.length)} of {filteredRows.length.toLocaleString()}
          </span>
        </div>
      )}
    </div>
