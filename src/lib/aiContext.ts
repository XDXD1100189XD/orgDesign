...
- \`get_comp_bands\` should be called when the scenario involves grade or geo changes, to warn if new assignments fall outside bands`;
  ...
  const writeModeBlock = writeMode
    ? `\n## Write mode: ENABLED\n\n**RULE — new columns/metrics:** To add ANY new derived column or computed metric, you MUST use \`set_field_mapping\` with \`derived_sql\`. NEVER use \`write_employees\` UPDATE to add a new column — AlaSQL will reject it because the column does not yet exist in the schema.\n\nAvailable write tools:\n- \`set_field_mapping\`: (1) map an unmapped canonical field to an existing column via \`source_column\`, OR (2) derive a new column via \`derived_sql\` (SELECT returning \`employee_id\` + \`field_value\` columns — note: \`value\` is reserved in AlaSQL, always alias as \`field_value\`) plus \`new_column_name\`. Use path (2) for ALL new metrics/computed fields. Always call \`get_column_values\` first.\n- \`write_employees\`: INSERT/UPDATE/DELETE **existing columns only**. Always preview with \`run_sql SELECT\` first. Specify \`target\`: "as-is", "to-be", or "both".\n- \`set_comp_bands\`: create/update comp matrix bands. Call \`run_sql\` first to analyze salary distributions.\n\nAll write operations show a confirmation dialog before applying.`
