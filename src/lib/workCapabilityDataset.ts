...
export type WorkCapabilityValidationStatus = "success" | "failed" | "imported_with_warnings";
export type WorkCapabilityNormalizationStatus = "auto_applied" | "needs_review" | "approved" | "ignored";
...
export type TaxonomyMutation =
  ...
  | { type: "move_category"; processArea: string; category: string; toProcessArea: string }
  | { type: "move_activity"; activityId: string; processArea: string; category: string }
  | { type: "update_activity"; activityId: string; patch: Partial<ActivityLibraryRow> }
  ...
  | { type: "merge_activities"; sourceActivityIds: string[]; targetActivityId: string };
...
export type WorkCapabilityTaxonomyGraphNodeKind = "root" | "domain" | "category" | "process" | "activity";
...
const SPECS: Record<WorkCapabilityDatasetType, DatasetSpec> = {
  activity_library: {
    ...
    fields: {
      ...
      nature: { type: "enum", required: true, values: ["Strategic", "Operational", "Transactional", "Governance"] },
      criticality: { type: "enum", required: true, values: ["Low", "Medium", "High"] },
      automation_cost_reduction_pct: { type: "number", required: true },
      outsourcing_cost_reduction_pct: { type: "number", required: true },
      complexity: { type: "enum", required: true, values: ["Low", "Medium", "High"] },
      frequency: { type: "enum", required: true, values: ["Daily", "Weekly", "Monthly", "Quarterly", "Ad hoc"] },
      active_flag: { type: "boolean", required: true },
    },
  },
  activity_assignments: {
    primaryKey: "assignment_id",
    requiredHeaders: ["assignment_id", "employee_id", "activity_id", "time_allocation_pct", "accountability", "assignment_source", "validated"],
    fields: {
      assignment_id: { type: "string", required: true },
      employee_id: { type: "string", required: true },
      activity_id: { type: "string", required: true },
      time_allocation_pct: { type: "number", required: true },
      accountability: { type: "enum", required: true, values: ["Responsible", "Accountable", "Consulted", "Informed"] },
      assignment_source: { type: "enum", required: true, values: ["Synthetic", "Manual", "AI-Suggested", "Imported"] },
  ...
  skill_library: {
    primaryKey: "skill_id",
    requiredHeaders: ["skill_id", "skill_name", "skill_family", "skill_category", "skill_type", "criticality", "proficiency_scale", "active_flag"],
    fields: {
      skill_id: { type: "string", required: true },
      skill_name: { type: "string", required: true },
      skill_family: { type: "string", required: true },
      skill_category: { type: "string", required: true },
      skill_type: { type: "enum", required: true, values: ["Technical", "Functional", "Leadership", "Analytical", "Process", "Behavioral"] },
      criticality: { type: "enum", required: true, values: ["Low", "Medium", "High"] },
  ...
  role_skill_requirements: {
    ...
    requiredHeaders: ["requirement_id", "role_key", "position_title", "department", "skill_id", "required_level", "criticality", "requirement_source", "validated", "active_flag"],
    fields: {
      ...
      role_level: { type: "enum", values: ["Executive", "Director", "Manager", "Lead", "Individual Contributor", "Analyst", "Specialist"] },
      skill_id: { type: "string", required: true },
      required_level: { type: "integer", required: true, min: 1, max: 5 },
      criticality: { type: "enum", required: true, values: ["Low", "Medium", "High"] },
      requirement_source: { type: "enum", required: true, values: ["Synthetic", "Manual", "AI-Suggested", "Imported"] },
      validated: { type: "boolean", required: true },
      active_flag: { type: "boolean", required: true },
    },
  },
  employee_skills: {
    ...
    requiredHeaders: ["employee_skill_id", "employee_id", "skill_id", "current_level", "evidence_source", "validated", "synthetic_flag"],
    fields: {
      ...
      gap_status: { type: "enum", values: ["Covered", "Minor gap", "Major gap"] },
      criticality: { type: "enum", values: ["Low", "Medium", "High"] },
      evidence_source: { type: "enum", required: true, values: ["Synthetic", "Manual", "Manager", "Certification", "AI-Suggested", "Imported"] },
      confidence_score: { type: "number" },
      validated: { type: "boolean", required: true },
  ...
  activity_skill_requirements: {
    ...
    requiredHeaders: ["activity_skill_requirement_id", "activity_id", "skill_id", "required_level", "criticality", "relationship_type", "weight", "requirement_source", "validated", "active_flag"],
    fields: {
      ...
      required_level: { type: "integer", required: true, min: 1, max: 5 },
      criticality: { type: "enum", required: true, values: ["Low", "Medium", "High"] },
      relationship_type: { type: "enum", required: true, values: ["Primary", "Supporting", "Governance", "Enabling"] },
...
const NORMALIZATION_ALIASES: Record<string, { value: string; confidence: number; reason: string }> = {
  ...
  "gap_status|not required / adjacent": {
    value: "Not Required",
    confidence: 0.95,
    reason: "Adjacent skills that are not required use a custom Not Required category.",
  },
  "relationship_type|optional": {
    value: "Optional",
    confidence: 0.95,
    reason: "Optional is a custom relationship category preserved for planning.",
  },
};
...
function byType(files: ParsedWorkCapabilityFile[]): Partial<Record<WorkCapabilityDatasetType, ParsedWorkCapabilityFile>> {
  const output: Partial<Record<WorkCapabilityDatasetType, ParsedWorkCapabilityFile>> = {};
  for (const file of files) {
    const detected = file.datasetType ?? detectWorkCapabilityDataset(file.headers);
...
export function detectWorkCapabilityDataset(headers: string[]): WorkCapabilityDatasetType | null {
  const headerSet = new Set(headers);
  let best: { type: WorkCapabilityDatasetType; score: number } | null = null;
  for (const type of DATASET_ORDER) {
    const score = SPECS[type].requiredHeaders.filter((header) => headerSet.has(header)).length;
...
function normalizationRecordId(type: WorkCapabilityDatasetType, field: string, rawValue: string): string {
  return `${type}:${field}:${encodeURIComponent(labelKey(rawValue))}`;
}

function enumFieldsFor(type: WorkCapabilityDatasetType): Array<[string, FieldSpec]> {
  return Object.entries(SPECS[type].fields).filter(([, spec]) => spec.type === "enum" && spec.values);
}

function emptyNormalization(records: WorkCapabilityNormalizationRecord[] = []): WorkCapabilityNormalizationSummary {
  return {
    totalGroups: records.length,
    autoApplied: records.filter((record) => record.status === "auto_applied").length,
...
function buildNormalizationSummary(
  filesByType: Partial<Record<WorkCapabilityDatasetType, ParsedWorkCapabilityFile>>,
    ...
    for (const [field, spec] of enumFieldsFor(type)) {
      ...
      rows.forEach((row, index) => {
        ...
        byId.set(id, {
          ...
          reason: alias?.reason ?? "No trusted normalization rule exists yet; review before using this value in analytics.",
        });
      });
    }
  }

  return emptyNormalization([...byId.values()].sort((a, b) => {
    const typeOrder = DATASET_ORDER.indexOf(a.datasetType) - DATASET_ORDER.indexOf(b.datasetType);
...
function normalizationByRecord(summary: WorkCapabilityNormalizationSummary): Map<string, WorkCapabilityNormalizationRecord> {
...
function normalizeRowsForType<T extends ExcelRow>(
  ...
  return clone(rows).map((row: ExcelRow) => {
    for (const [field, spec] of enumFieldsFor(type)) {
      const rawValue = textValue(row, field);
      row[`${field}${NORMALIZED_SUFFIX}`] = normalizedValueFor(type, field, rawValue, spec.values ?? [], records);
    }
    return row;
  }) as T[];
}

function applyNormalizationToDataset(dataset: WorkCapabilityDataset): WorkCapabilityDataset {
  return {
    ...dataset,
    shared: {
      activityLibrary: normalizeRowsForType<ActivityLibraryRow>("activity_library", dataset.shared.activityLibrary, dataset.normalization),
      skillLibrary: normalizeRowsForType<SkillLibraryRow>("skill_library", dataset.shared.skillLibrary, dataset.normalization),
      activitySkillRequirements: normalizeRowsForType<ActivitySkillRequirementRow>("activity_skill_requirements", dataset.shared.activitySkillRequirements, dataset.normalization),
    },
    states: {
      asIs: {
        ...dataset.states.asIs,
        activityAssignments: normalizeRowsForType<ActivityAssignmentRow>("activity_assignments", dataset.states.asIs.activityAssignments, dataset.normalization),
        roleSkillRequirements: normalizeRowsForType<RoleSkillRequirementRow>("role_skill_requirements", dataset.states.asIs.roleSkillRequirements, dataset.normalization),
        employeeSkills: normalizeRowsForType<EmployeeSkillRow>("employee_skills", dataset.states.asIs.employeeSkills, dataset.normalization),
      },
      toBe: dataset.states.toBe
        ? {
            ...dataset.states.toBe,
            activityAssignments: normalizeRowsForType<ActivityAssignmentRow>("activity_assignments", dataset.states.toBe.activityAssignments, dataset.normalization),
            roleSkillRequirements: normalizeRowsForType<RoleSkillRequirementRow>("role_skill_requirements", dataset.states.toBe.roleSkillRequirements, dataset.normalization),
            employeeSkills: normalizeRowsForType<EmployeeSkillRow>("employee_skills", dataset.states.toBe.employeeSkills, dataset.normalization),
          }
        : undefined,
    },
  };
...
function stateAssignmentRows(dataset: WorkCapabilityDataset): ActivityAssignmentRow[] {
  return [
...
function countAssignmentsForActivities(dataset: WorkCapabilityDataset, activityIds: Set<string>): number {
  return stateAssignmentRows(dataset).filter((row) => activityIds.has(textValue(row, "activity_id"))).length;
}

function countSkillRequirementsForActivities(dataset: WorkCapabilityDataset, activityIds: Set<string>): number {
  return dataset.shared.activitySkillRequirements.filter((row) => activityIds.has(textValue(row, "activity_id"))).length;
}

function requireActivity(dataset: WorkCapabilityDataset, activityId: string): ActivityLibraryRow {
  const activity = dataset.shared.activityLibrary.find((row) => textValue(row, "activity_id") === activityId);
  if (!activity) throw new Error(`activity_id ${activityId} does not exist in activity_library`);
  return activity;
}

function taxonomyImpact(activityRows: number, assignmentRows: number, skillRequirementRows: number): WorkCapabilityTaxonomyMutationImpact {
  return {
    activityRows,
    assignmentRows,
...
export function getWorkCapabilityTaxonomyMutationImpact(
  ...
  if (mutation.type === "rename_category") {
    const activityIds = new Set(
      dataset.shared.activityLibrary
        .filter((row) => textValue(row, "process_area") === mutation.processArea && textValue(row, "activity_category") === mutation.from)
        .map((row) => textValue(row, "activity_id")),
    );
    return taxonomyImpact(activityIds.size, 0, 0);
  }
  ...
  if (mutation.type === "move_category") {
    const activityIds = new Set(
      dataset.shared.activityLibrary
        .filter((row) => textValue(row, "process_area") === mutation.processArea && textValue(row, "activity_category") === mutation.category)
        .map((row) => textValue(row, "activity_id")),
    );
    return taxonomyImpact(activityIds.size, 0, 0);
  }
  ...
  if (mutation.type === "move_activity" || mutation.type === "update_activity") {
  ...
  if (mutation.type === "merge_activities") {
    ...
    const sourceIds = new Set(mutation.sourceActivityIds.filter((id) => id !== mutation.targetActivityId));
...
export function canHardDeleteWorkCapabilityActivity(
  ...
  const skillRequirementRows = countSkillRequirementsForActivities(dataset, activityIds);
  if (assignmentRows > 0 || skillRequirementRows > 0) {
    return {
      allowed: false,
      reason: "Hard delete is blocked because dependent rows exist; deactivate or merge the activity instead.",
      assignmentRows,
      skillRequirementRows,
    };
  }
...
export function workCapabilityCategoryNodeId(domain: string, category: string): string {
  return `category::${taxonomySegment(domain)}::${taxonomySegment(category)}`;
}

export function workCapabilityProcessNodeId(domain: string, category: string, processArea: string): string {
...
function deriveWorkDomain(row: ActivityLibraryRow): string {
  ...
  if (departmentFocus && departmentFocus.toLowerCase() !== "all") return departmentFocus;
  ...
  if (/\b(data|analytics|report|reporting|dashboard|insight|bi)\b/.test(haystack)) return "Data & Analytics";
  if (/\b(transition|kt|knowledge transfer|vendor|service transfer)\b/.test(haystack)) return "Transition Management";
  if (/\b(technology|engineering|application|development|platform|system)\b/.test(haystack)) return "Technology Operations";
  if (/\b(governance|risk|audit|compliance|control)\b/.test(haystack)) return "Governance & Risk";
  if (/\b(people|hr|talent|workforce|learning)\b/.test(haystack)) return "People & Culture";
  if (/\b(finance|budget|procurement|invoice|billing)\b/.test(haystack)) return "Finance";
  if (/\b(support|admin|operations|service|customer)\b/.test(haystack)) return "Business Support";
...
export function buildWorkCapabilityTaxonomyGraph(
  ...
  const sourceRows = dataset.shared.activityLibrary.filter((row) => options.showInactive || activityIsActive(row));
  const matchedActivityIds = new Set(
    query ? sourceRows.filter((row) => taxonomyMatches(row, query)).map((row) => textValue(row, "activity_id")) : [],
  );

  const byDomain = new Map<string, Map<string, Map<string, ActivityLibraryRow[]>>>();
  for (const activity of sourceRows) {
    if (query && matchedActivityIds.size > 0 && !taxonomyMatches(activity, query)) continue;
    const domain = deriveWorkDomain(activity);
    const category = normalizeTaxonomyLabel(textValue(activity, "activity_category"), "Uncategorized");
    const processArea = normalizeTaxonomyLabel(textValue(activity, "process_area"), "General Process");
  ...
  const nodes: WorkCapabilityTaxonomyGraphNode[] = [{
    ...
    categoryCount: Array.from(byDomain.values()).reduce((sum, categories) => sum + categories.size, 0),
    processCount: Array.from(byDomain.values()).reduce(
      (sum, categories) => sum + Array.from(categories.values()).reduce((inner, processes) => inner + processes.size, 0),
      0,
    ),
    active: true,
  ...
  let domainOrder = 0;
  const sortedDomains = Array.from(byDomain.entries()).sort(([a], [b]) => a.localeCompare(b));
  for (const [domain, categories] of sortedDomains) {
    const domainId = workCapabilityDomainNodeId(domain);
    const domainActivities = Array.from(categories.values()).flatMap((processes) => Array.from(processes.values()).flat());
    ...
    nodes.push({
      ...
      processCount: Array.from(categories.values()).reduce((sum, processes) => sum + processes.size, 0),
      criticality: highestCriticality(domainActivities),
      active: true,
      selected: domainSelected,
      expanded: true,
    });
    edges.push({ id: `taxonomy-root->${domainId}`, source: "taxonomy-root", target: domainId });
    ...
    for (const [category, processes] of sortedCategories) {
      ...
      const categoryHasSelectedActivity = categoryActivities.some((row) => selectedNodeId === workCapabilityActivityNodeId(textValue(row, "activity_id")));
      const expandCategory = query.length > 0 || categorySelected || categoryHasSelectedActivity || Array.from(processes.keys()).some((processArea) => selectedNodeId === workCapabilityProcessNodeId(domain, category, processArea));
      nodes.push({
        id: categoryId,
        kind: "category",
        label: category,
      ...
      edges.push({ id: `${domainId}->${categoryId}`, source: domainId, target: categoryId });

      let processOrder = 0;
      for (const [processArea, activities] of Array.from(processes.entries()).sort(([a], [b]) => a.localeCompare(b))) {
        const processId = workCapabilityProcessNodeId(domain, category, processArea);
        const processSelected = selectedNodeId === processId;
        const processHasSelectedActivity = activities.some((row) => selectedNodeId === workCapabilityActivityNodeId(textValue(row, "activity_id")));
        const expandProcess = query.length > 0 || categorySelected || processSelected || processHasSelectedActivity;
        nodes.push({
          id: processId,
          kind: "process",
          label: processArea,
        ...
        edges.push({ id: `${categoryId}->${processId}`, source: categoryId, target: processId });

        if (!expandProcess) continue;
        activities
          .slice()
          .sort((a, b) => textValue(a, "activity_name").localeCompare(textValue(b, "activity_name")))
          .forEach((activity, index) => {
            ...
            nodes.push({
              ...
              automationPct: normalizeNumber(activity.automation_cost_reduction_pct) ?? 0,
              outsourcingPct: normalizeNumber(activity.outsourcing_cost_reduction_pct) ?? 0,
              active: activityIsActive(activity),
              selected: selectedNodeId === nodeId,
              expanded: false,
            });
            edges.push({ id: `${processId}->${nodeId}`, source: processId, target: nodeId });
          });
      }
    }
  }
...
function mappedColumn(mapping: ColumnMapping | null | undefined, field: string): string | null {
  return mapping?.[field as keyof ColumnMapping]?.column ?? null;
}

function firstText(row: ExcelRow | undefined, fields: Array<string | null | undefined>): string {
  if (!row) return "";
...
function firstNumber(row: ExcelRow | undefined, fields: Array<string | null | undefined>, fallback = 0): number {
...
function riskForCoverage(activityCriticality: string, coverages: Array<number | null>): WorkCapabilitySkillRisk {
  if (coverages.length === 0 || coverages.some((coverage) => coverage == null)) return "Unknown";
  if (activityCriticality === "High" && coverages.some((coverage) => (coverage ?? 0) < 50)) return "High";
...
export function buildWorkCapabilityActivityPortfolio(
  ...
  const orgById = new Map(input.orgRows.map((row) => [textValue(row, input.orgEmployeeIdColumn), row]));
  ...
  const skillsById = new Map(input.dataset.shared.skillLibrary.map((row) => [textValue(row, "skill_id"), row]));
  ...
  for (const skillRow of employeeSkills) {
    ...
    if (!skillRowsByEmployee.has(employeeId)) skillRowsByEmployee.set(employeeId, []);
    skillRowsByEmployee.get(employeeId)!.push(skillRow);
  }

  const nameFields = orgColumnCandidates(input.columnMapping, "Full Name", ["full_name", "name", "employee_name"]);
  const roleFields = orgColumnCandidates(input.columnMapping, "Business Title", ["position_title", "title", "business_title"]);
  const departmentFields = orgColumnCandidates(input.columnMapping, "Department Name", ["department", "department_name", "dept"]);
  const gradeFields = orgColumnCandidates(input.columnMapping, "Compensation Grade", ["grade", "compensation_grade", "level"]);
  const costFields = [
  ...
  const fteFields = orgColumnCandidates(input.columnMapping, "FTE", ["FTE", "fte"]);
  ...
  const activities = input.dataset.shared.activityLibrary
    ...
    .map((activity): WorkCapabilityActivityMetric => {
      const activityId = textValue(activity, "activity_id");
      const activityAssignments = assignments.filter((row) => textValue(row, "activity_id") === activityId);
      ...
      const deliveryFootprint = activityAssignments.map((assignment): WorkCapabilityDeliveryFootprintRow => {
        const employeeId = textValue(assignment, "employee_id");
        const orgRow = orgById.get(employeeId);
        const timeAllocationPct = normalizeNumber(assignment.time_allocation_pct) ?? 0;
        ...
        const department = firstText(orgRow, departmentFields) || textValue(assignment, "department");
        ...
        return {
          ...
          employeeId,
          employeeName: firstText(orgRow, nameFields) || textValue(assignment, "employee_name"),
          role: firstText(orgRow, roleFields) || textValue(assignment, "position_title"),
          department,
          grade: firstText(orgRow, gradeFields) || textValue(assignment, "grade"),
          timeAllocationPct,
          costContribution,
          fteContribution,
          accountability,
      ...
      const requirements = input.dataset.shared.activitySkillRequirements.filter((row) => textValue(row, "activity_id") === activityId && activityIsActive(row));
      const requiredSkills = requirements.map((requirement): WorkCapabilityRequiredSkillRow => {
        const skillId = textValue(requirement, "skill_id");
        const requiredLevel = normalizeNumber(requirement.required_level) ?? 0;
        ...
        for (const employeeId of assignedEmployeeIds) {
          const employeeSkill = (skillRowsByEmployee.get(employeeId) ?? []).find((row) => textValue(row, "skill_id") === skillId);
          if (!employeeSkill) continue;
          employeesWithData += 1;
          if ((normalizeNumber(employeeSkill.current_level) ?? 0) >= requiredLevel) coveredEmployees += 1;
        }

        const coveragePct = assignedEmployeeIds.length === 0 || employeesWithData < assignedEmployeeIds.length
          ? null
          : coveredEmployees / assignedEmployeeIds.length * 100;
        const gap = coveragePct == null
          ? "Unknown"
          : coveragePct < 50
            ? "High"
            : coveragePct < 80
        ...
        return {
          skillId,
          skillName: firstText(skillsById.get(skillId), ["skill_name"]) || textValue(requirement, "skill_name") || skillId,
          requiredLevel,
          coveragePct,
          gap,
        };
      ...
      const automationCostReductionPct = normalizeNumber(activity.automation_cost_reduction_pct) ?? 0;
      const outsourcingCostReductionPct = normalizeNumber(activity.outsourcing_cost_reduction_pct) ?? 0;
      const skillRisk = riskForCoverage(textValue(activity, "criticality"), requiredSkills.map((skill) => skill.coveragePct));

      return {
        activityId,
        activityName: textValue(activity, "activity_name") || activityId,
        domain: deriveWorkDomain(activity),
        processArea: normalizeTaxonomyLabel(textValue(activity, "process_area"), "General Process"),
        category: normalizeTaxonomyLabel(textValue(activity, "activity_category"), "Uncategorized"),
        nature: textValue(activity, "nature"),
        criticality: textValue(activity, "criticality"),
    ...
    .sort((a, b) => b.totalCost - a.totalCost || a.activityName.localeCompare(b.activityName));
  ...
  for (const [domain, domainActivities] of Array.from(domainMap.entries()).sort(([a], [b]) => a.localeCompare(b))) {
    const domainCategories = new Set(domainActivities.map((activity) => activity.category));
    const domainProcesses = new Set(domainActivities.map((activity) => `${activity.category}::${activity.processArea}`));
    tree.push({
      ...
      criticalActivityCount: domainActivities.filter((activity) => activity.criticality === "High").length,
      totalCost: domainActivities.reduce((sum, activity) => sum + activity.totalCost, 0),
      totalFTE: domainActivities.reduce((sum, activity) => sum + activity.totalFTE, 0),
    });

    const categoryMap = new Map<string, WorkCapabilityActivityMetric[]>();
    for (const activity of domainActivities) {
      if (!categoryMap.has(activity.category)) categoryMap.set(activity.category, []);
      categoryMap.get(activity.category)!.push(activity);
    }

    for (const [category, categoryActivities] of Array.from(categoryMap.entries()).sort(([a], [b]) => a.localeCompare(b))) {
      const categoryProcesses = new Map<string, WorkCapabilityActivityMetric[]>();
      for (const activity of categoryActivities) {
        if (!categoryProcesses.has(activity.processArea)) categoryProcesses.set(activity.processArea, []);
      ...
      tree.push({
        ...
        criticalActivityCount: categoryActivities.filter((activity) => activity.criticality === "High").length,
        totalCost: categoryActivities.reduce((sum, activity) => sum + activity.totalCost, 0),
        totalFTE: categoryActivities.reduce((sum, activity) => sum + activity.totalFTE, 0),
      });

      for (const [processArea, processActivities] of Array.from(categoryProcesses.entries()).sort(([a], [b]) => a.localeCompare(b))) {
        tree.push({
          ...
          categoryCount: 0,
          processCount: 0,
          criticalActivityCount: processActivities.filter((activity) => activity.criticality === "High").length,
          totalCost: processActivities.reduce((sum, activity) => sum + activity.totalCost, 0),
          totalFTE: processActivities.reduce((sum, activity) => sum + activity.totalFTE, 0),
        });
      }
    }
  }
  ...
  const departments = new Set<string>();
  for (const activity of activities) {
    for (const row of activity.deliveryFootprint) if (row.department) departments.add(row.department);
  }

  return {
    kpis: {
      totalActivities: activities.length,
      criticalActivities: activities.filter((activity) => activity.criticality === "High").length,
      mappedCost: activities.reduce((sum, activity) => sum + activity.totalCost, 0),
      mappedFTE: activities.reduce((sum, activity) => sum + activity.totalFTE, 0),
      ownershipGaps: activities.filter((activity) => activity.accountableCount === 0).length,
      automationSaving: activities.reduce((sum, activity) => sum + activity.automationSaving, 0),
    },
    tree,
    activities,
    filterOptions: {
      departments: Array.from(departments).sort(),
      domains: Array.from(new Set(activities.map((activity) => activity.domain))).sort(),
      processAreas: Array.from(new Set(activities.map((activity) => activity.processArea))).sort(),
      categories: Array.from(new Set(activities.map((activity) => activity.category))).sort(),
      criticalities: Array.from(new Set(activities.map((activity) => activity.criticality).filter(Boolean))).sort(),
      natures: Array.from(new Set(activities.map((activity) => activity.nature).filter(Boolean))).sort(),
      skillRisks: Array.from(new Set(activities.map((activity) => activity.skillRisk))).sort() as WorkCapabilitySkillRisk[],
    },
  };
}
...
export function getWorkCapabilityArchiveImpact(input: {
  ...
  const selectedActivities = input.portfolio.activities.filter((activity) => activityIds.has(activity.activityId));
  ...
  return {
    ...
    totalCost: selectedActivities.reduce((sum, activity) => sum + activity.totalCost, 0),
    totalFTE: selectedActivities.reduce((sum, activity) => sum + activity.totalFTE, 0),
    skillRequirements: input.dataset.shared.activitySkillRequirements.filter((row) => activityIds.has(textValue(row, "activity_id"))).length,
    accountabilityLinks,
  };
}
...
export function getWorkCapabilityMergeImpact(input: {
  ...
  const sourceIds = new Set(input.sourceActivityIds.filter((id) => id && id !== input.targetActivityId));
  const relevantSkillRequirements = input.dataset.shared.activitySkillRequirements
    .filter((row) => sourceIds.has(textValue(row, "activity_id")) || textValue(row, "activity_id") === input.targetActivityId)
    .map((row) => ({
      activity_id: sourceIds.has(textValue(row, "activity_id")) ? input.targetActivityId : textValue(row, "activity_id"),
      skill_id: textValue(row, "skill_id"),
    }));
  const skillKeys = new Set(relevantSkillRequirements.map((row) => `${row.activity_id}::${row.skill_id}`));

  return {
    assignmentsMoved: input.dataset.states.asIs.activityAssignments.filter((row) => sourceIds.has(textValue(row, "activity_id"))).length,
    skillRequirementsMerged: input.dataset.shared.activitySkillRequirements.filter((row) => sourceIds.has(textValue(row, "activity_id"))).length,
    duplicateSkillRequirementsDeduped: Math.max(0, relevantSkillRequirements.length - skillKeys.size),
...
export function buildWorkCapabilityTaxonomyCleanupSuggestions(input: {
  ...
  const activeActivities = input.portfolio.activities.filter((activity) => activity.active);
  ...
  for (const [categoryKey, categoryActivities] of activeCategoryMap) {
    const processes = new Set(categoryActivities.map((activity) => activity.processArea));
    ...
    const activityIds = categoryActivities.map((activity) => activity.activityId);
    suggestions.push({
      ...
      description: "This branch has one category, one process, and one activity. It may be over-classified.",
      activityIds,
  ...
  const unassigned = activeActivities.filter((activity) => activity.assignedPeople === 0);
  if (unassigned.length > 0) {
    suggestions.push({
      ...
      description: "These activities are in the taxonomy but have no delivery footprint yet.",
  ...
  const missingSkills = activeActivities.filter((activity) => activity.requiredSkills.length === 0);
  if (missingSkills.length > 0) {
    suggestions.push({
      ...
      description: "Add skill requirements before relying on capability risk views for these activities.",
  ...
  const ownerGaps = activeActivities.filter((activity) => activity.accountableCount === 0);
  ...
  const archived = input.dataset.shared.activityLibrary.filter((row) => !activityIsActive(row));
  if (archived.length > 0) {
    suggestions.push({
      ...
      description: "Turn on Show Archived to review or restore archived taxonomy rows.",
  ...
  return suggestions.sort((a, b) => criticalityRank(b.severity) - criticalityRank(a.severity) || b.count - a.count);
}

function coerceActivityPatch(patch: Partial<ActivityLibraryRow>): Partial<ActivityLibraryRow> {
  const next: Partial<ActivityLibraryRow> = { ...patch };
  if (next.automation_cost_reduction_pct != null) {
    next.automation_cost_reduction_pct = normalizeNumber(next.automation_cost_reduction_pct) ?? 0;
  }
  if (next.outsourcing_cost_reduction_pct != null) {
    next.outsourcing_cost_reduction_pct = normalizeNumber(next.outsourcing_cost_reduction_pct) ?? 0;
  }
  if (next.active_flag != null) {
    next.active_flag = normalizeBoolean(next.active_flag) ?? Boolean(next.active_flag);
  }
  return next;
}

function mutationTarget(mutation: TaxonomyMutation): string {
  if (mutation.type === "rename_process") return `${mutation.from} -> ${mutation.to}`;
  if (mutation.type === "rename_category") return `${mutation.processArea} / ${mutation.from} -> ${mutation.to}`;
  if (mutation.type === "move_category") return `${mutation.processArea} / ${mutation.category} -> ${mutation.toProcessArea}`;
  ...
  if (mutation.type === "deactivate_activities") return mutation.activityIds.join(", ");
  return `${mutation.sourceActivityIds.join(", ")} -> ${mutation.targetActivityId}`;
}

function syncTaxonomyLoadedCount(dataset: WorkCapabilityDataset): WorkCapabilityDataset {
  return {
    ...dataset,
    validation: {
      ...dataset.validation,
      loaded: {
        ...
        activity_skill_requirements: dataset.shared.activitySkillRequirements.length,
      },
    },
  };
}
...
function dedupeMergedAssignments(rows: ActivityAssignmentRow[]): ActivityAssignmentRow[] {
  ...
  for (const row of rows) {
    ...
    if (!existing) {
      byKey.set(key, { ...row, time_allocation_pct: normalizeNumber(row.time_allocation_pct) ?? 0 });
      continue;
    }
    byKey.set(key, {
      ...existing,
      time_allocation_pct: (normalizeNumber(existing.time_allocation_pct) ?? 0) + (normalizeNumber(row.time_allocation_pct) ?? 0),
...
function dedupeMergedSkillRequirements(rows: ActivitySkillRequirementRow[]): ActivitySkillRequirementRow[] {
  const byKey = new Map<string, ActivitySkillRequirementRow>();
  for (const row of rows) {
    const key = [textValue(row, "activity_id"), textValue(row, "skill_id")].join("::");
    const existing = byKey.get(key);
    if (!existing) {
      byKey.set(key, {
        ...row,
    ...
    byKey.set(key, {
      ...existing,
      required_level: Math.max(normalizeNumber(existing.required_level) ?? 0, normalizeNumber(row.required_level) ?? 0),
      criticality: highestCriticalityValue(textValue(existing, "criticality"), textValue(row, "criticality")),
      weight: Math.max(normalizeNumber(existing.weight) ?? 0, normalizeNumber(row.weight) ?? 0),
    });
  }
  return Array.from(byKey.values());
}
...
export function applyWorkCapabilityTaxonomyMutation(
  ...
  if (mutation.type === "rename_process") {
    ...
    next.shared.activityLibrary = next.shared.activityLibrary.map((row: ActivityLibraryRow) =>
      textValue(row, "process_area") === mutation.from ? { ...row, process_area: to } : row,
    );
  } else if (mutation.type === "rename_category") {
    const to = mutation.to.trim();
    if (!to) throw new Error("Activity category name is required");
    next.shared.activityLibrary = next.shared.activityLibrary.map((row: ActivityLibraryRow) =>
      textValue(row, "process_area") === mutation.processArea && textValue(row, "activity_category") === mutation.from
    ...
    next.shared.activityLibrary = next.shared.activityLibrary.map((row: ActivityLibraryRow) =>
      textValue(row, "process_area") === mutation.processArea && textValue(row, "activity_category") === mutation.category
    ...
    if (!processArea || !category) throw new Error("Process area and category are required");
    next.shared.activityLibrary = next.shared.activityLibrary.map((row: ActivityLibraryRow) =>
    ...
    next.shared.activityLibrary = next.shared.activityLibrary.map((row: ActivityLibraryRow) =>
      textValue(row, "activity_id") === mutation.activityId
        ? { ...row, ...coerceActivityPatch(mutation.patch), activity_id: row.activity_id }
        : row,
    );
    ...
    if (next.shared.activityLibrary.some((row: ActivityLibraryRow) => textValue(row, "activity_id") === activityId)) {
      throw new Error(`activity_id ${activityId} already exists in activity_library`);
    }
    ...
    next.shared.activityLibrary = next.shared.activityLibrary.map((row: ActivityLibraryRow) =>
      textValue(row, "activity_id") === mutation.activityId ? { ...row, active_flag: false } : row,
    );
  } else if (mutation.type === "deactivate_activities") {
    const activityIds = new Set(mutation.activityIds);
    next.shared.activityLibrary = next.shared.activityLibrary.map((row: ActivityLibraryRow) =>
      activityIds.has(textValue(row, "activity_id")) ? { ...row, active_flag: false, last_updated_at: now } : row,
    );
  } else if (mutation.type === "merge_activities") {
    const sourceIds = new Set(mutation.sourceActivityIds.filter((id) => id !== mutation.targetActivityId));
    const mergedActivityNames = next.shared.activityLibrary
      .filter((row: ActivityLibraryRow) => sourceIds.has(textValue(row, "activity_id")))
      .map((row: ActivityLibraryRow) => textValue(row, "activity_name") || textValue(row, "activity_id"));
    next.shared.activityLibrary = next.shared.activityLibrary.map((row: ActivityLibraryRow) =>
      sourceIds.has(textValue(row, "activity_id"))
        ? { ...row, active_flag: false, merged_into_activity_id: mutation.targetActivityId, last_updated_at: now }
        : row,
    );
    next.shared.activitySkillRequirements = dedupeMergedSkillRequirements(
      next.shared.activitySkillRequirements.map((row: ActivitySkillRequirementRow) =>
        sourceIds.has(textValue(row, "activity_id")) ? { ...row, activity_id: mutation.targetActivityId } : row,
      ),
    );
    ...
    if (next.states.toBe) {
      next.states.toBe.activityAssignments = dedupeMergedAssignments(
        next.states.toBe.activityAssignments.map((row: ActivityAssignmentRow) =>
          sourceIds.has(textValue(row, "activity_id")) ? { ...row, activity_id: mutation.targetActivityId } : row,
        ),
      );
...
function validateScalar(
  ...
  if (spec.required && isBlank(value)) {
    if (type === "activity_library" && ["department_focus", "activity_category", "process_area"].includes(field)) return;
  ...
  if (spec.type === "number" || spec.type === "integer") {
    ...
    if (parsed == null || (spec.type === "integer" && !Number.isInteger(parsed))) {
      errors.push(`${type} row ${rowNumber(index)}: ${field} must be a ${spec.type === "integer" ? "integer" : "number"}`);
      return;
    }
    if (spec.min != null && parsed < spec.min) errors.push(`${type} row ${rowNumber(index)}: ${field} must be at least ${spec.min}`);
    if (spec.max != null && parsed > spec.max) errors.push(`${type} row ${rowNumber(index)}: ${field} must be at most ${spec.max}`);
  }
...
function validateRows(type: WorkCapabilityDatasetType, file: ParsedWorkCapabilityFile, errors: string[], warnings: string[]): void {
  const spec = SPECS[type];
  const headers = new Set(file.headers);
  for (const header of spec.requiredHeaders) {
    if (!headers.has(header)) errors.push(`${type}: missing required column ${header}`);
  }
  ...
  file.rows.forEach((row, index) => {
    ...
    if ("confidence_score" in spec.fields && isBlank(row.confidence_score)) {
      warnings.push(`${type} row ${rowNumber(index)}: confidence_score is missing`);
    }
  });

}

function validateRelationships(filesByType: Partial<Record<WorkCapabilityDatasetType, ParsedWorkCapabilityFile>>, orgRows: ExcelRow[], orgEmployeeIdColumn: string, errors: string[], warnings: string[]): void {
  const orgIds = new Set(orgRows.map((row) => textValue(row, orgEmployeeIdColumn)).filter(Boolean));
  const activities = new Set((filesByType.activity_library?.rows ?? []).map((row) => textValue(row, "activity_id")).filter(Boolean));
  const skills = new Set((filesByType.skill_library?.rows ?? []).map((row) => textValue(row, "skill_id")).filter(Boolean));
  ...
  (filesByType.activity_assignments?.rows ?? []).forEach((row, index) => {
    ...
    if (employeeId && !orgIds.has(employeeId)) errors.push(`activity_assignments row ${rowNumber(index)}: employee_id ${employeeId} does not exist in As-Is org rows`);
    if (activityId && !activities.has(activityId)) errors.push(`activity_assignments row ${rowNumber(index)}: activity_id ${activityId} does not exist in activity_library`);
    const orgRow = orgRows.find((candidate) => textValue(candidate, orgEmployeeIdColumn) === employeeId);
    if (orgRow && !isBlank(row.employee_name) && !isBlank(orgRow.name) && textValue(row, "employee_name") !== textValue(orgRow, "name")) {
      warnings.push(`activity_assignments row ${rowNumber(index)}: employee_name differs from org data`);
    }
  });
  ...
  (filesByType.employee_skills?.rows ?? []).forEach((row, index) => {
    ...
    if (employeeId && !orgIds.has(employeeId)) errors.push(`employee_skills row ${rowNumber(index)}: employee_id ${employeeId} does not exist in As-Is org rows`);
    if (skillId && !skills.has(skillId)) errors.push(`employee_skills row ${rowNumber(index)}: skill_id ${skillId} does not exist in skill_library`);
  });

  (filesByType.role_skill_requirements?.rows ?? []).forEach((row, index) => {
    const skillId = textValue(row, "skill_id");
    if (skillId && !skills.has(skillId)) errors.push(`role_skill_requirements row ${rowNumber(index)}: skill_id ${skillId} does not exist in skill_library`);
  });

  (filesByType.activity_skill_requirements?.rows ?? []).forEach((row, index) => {
    const activityId = textValue(row, "activity_id");
    const skillId = textValue(row, "skill_id");
    if (activityId && !activities.has(activityId)) errors.push(`activity_skill_requirements row ${rowNumber(index)}: activity_id ${activityId} does not exist in activity_library`);
    if (skillId && !skills.has(skillId)) errors.push(`activity_skill_requirements row ${rowNumber(index)}: skill_id ${skillId} does not exist in skill_library`);
  });
}
...
export function validateWorkCapabilityFiles(input: {
  ...
  validateRelationships(filesByType, input.orgRows, input.orgEmployeeIdColumn, errors, warnings);
  ...
  return {
    status: errors.length > 0 ? "failed" : warnings.length > 0 ? "imported_with_warnings" : "success",
    loaded,
    errors,
    warnings,
    normalization,
...
export function createWorkCapabilityDataset(input: {
  ...
  const dataset: WorkCapabilityDataset = {
    ...
    shared: {
      activityLibrary: normalizeRowsForType<ActivityLibraryRow>("activity_library", filesByType.activity_library?.rows ?? [], normalization),
      skillLibrary: normalizeRowsForType<SkillLibraryRow>("skill_library", filesByType.skill_library?.rows ?? [], normalization),
      activitySkillRequirements: normalizeRowsForType<ActivitySkillRequirementRow>("activity_skill_requirements", filesByType.activity_skill_requirements?.rows ?? [], normalization),
    },
    states: {
      asIs: {
        stateId: "asIs",
        activityAssignments: normalizeRowsForType<ActivityAssignmentRow>("activity_assignments", filesByType.activity_assignments?.rows ?? [], normalization),
        roleSkillRequirements: normalizeRowsForType<RoleSkillRequirementRow>("role_skill_requirements", filesByType.role_skill_requirements?.rows ?? [], normalization),
        employeeSkills: normalizeRowsForType<EmployeeSkillRow>("employee_skills", filesByType.employee_skills?.rows ?? [], normalization),
        revision: 0,
...
export function updateWorkCapabilityNormalization(
  dataset: WorkCapabilityDataset,
  input: { id: string; status: "approved" | "ignored"; normalizedValue?: string },
): WorkCapabilityDataset {
  const records = dataset.normalization.records.map((record) => {
    ...
    const normalizedValue = (input.normalizedValue ?? record.suggestedValue ?? "").trim();
    return {
      ...record,
      status: "approved" as const,
      suggestedValue: normalizedValue || record.suggestedValue,
      normalizedValue,
      confidence: normalizedValue === record.suggestedValue ? record.confidence : 1,
      reason: normalizedValue === record.suggestedValue ? record.reason : "User-approved custom normalization.",
    };
  });
  ...
  const validation: WorkCapabilityValidationSummary = {
    ...dataset.validation,
    normalization,
    status: dataset.validation.errors.length > 0 ? "failed" : dataset.validation.warnings.length > 0 || normalization.totalGroups > 0 ? "imported_with_warnings" : "success",
  };
