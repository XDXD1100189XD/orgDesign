...
const orgRows = [
  { employee_id: "E1", name: "Ada", position_title: "CEO", department: "Exec", loaded_cost: 120, FTE: 1 },
  { employee_id: "E2", name: "Ben", position_title: "Manager", department: "Ops", loaded_cost: 80, FTE: 0.5 },
];
...
function allFiles(overrides = {}) {
  return [
    parsed("activity_library", overrides.activity_library ?? activityLibraryRows),
    parsed("activity_assignments", overrides.activity_assignments ?? activityAssignmentsRows),
    parsed("skill_library", overrides.skill_library ?? skillLibraryRows),
    parsed("role_skill_requirements", overrides.role_skill_requirements ?? roleSkillRequirementRows),
    parsed("employee_skills", overrides.employee_skills ?? employeeSkillRows),
    parsed("activity_skill_requirements", overrides.activity_skill_requirements ?? activitySkillRequirementRows),
  ];
}
...
test("rejects missing required columns", () => {
  const files = allFiles({
    activity_library: activityLibraryRows.map(({ activity_name, ...row }) => row),
  });

  const validation = validateWorkCapabilityFiles({ files, orgRows, orgEmployeeIdColumn: "employee_id" });
...
test("rejects duplicate primary keys", () => {
  const files = allFiles({
    skill_library: [skillLibraryRows[0], { ...skillLibraryRows[0], skill_name: "Duplicate" }],
  });
...
test("creates normalized companion fields while preserving raw imported labels", () => {
  const dataset = createWorkCapabilityDataset({
    orgDatasetId: "org_1",
    files: allFiles({
      activity_assignments: [{ ...activityAssignmentsRows[0], assignment_source: "Synthetic role/department rule" }],
      skill_library: [{ ...skillLibraryRows[0], skill_type: "Business" }],
      employee_skills: [{ ...employeeSkillRows[0], evidence_source: "Synthetic-Role-Inferred", gap_status: "Not required / adjacent" }],
      role_skill_requirements: [{ ...roleSkillRequirementRows[0], role_level: "Senior Leader", requirement_source: "Synthetic rule-based from title, department, grade" }],
      activity_skill_requirements: [{ ...activitySkillRequirementRows[0], relationship_type: "Optional", requirement_source: "Synthetic keyword + department rule" }],
    }),
    orgRows,
  ...
  assert.equal(dataset.states.asIs.activityAssignments[0].assignment_source, "Synthetic role/department rule");
  assert.equal(dataset.states.asIs.activityAssignments[0].assignment_source_normalized, "Synthetic");
  assert.equal(dataset.shared.skillLibrary[0].skill_type, "Business");
  assert.equal(dataset.shared.skillLibrary[0].skill_type_normalized, "Functional");
  assert.equal(dataset.states.asIs.roleSkillRequirements[0].role_level_normalized, "Executive");
  assert.equal(dataset.states.asIs.employeeSkills[0].gap_status_normalized, "Not Required");
  assert.equal(dataset.shared.activitySkillRequirements[0].relationship_type_normalized, "Optional");
  assert.equal(dataset.normalization.autoApplied, 8);
  assert.equal(dataset.normalization.needsReview, 0);
  assert.doesNotMatch(dataset.validation.warnings.join("\n"), /non-standard values "Synthetic role\/department rule"/i);
});
...
test("keeps unknown labels in normalization review until approved or ignored", () => {
  const dataset = createWorkCapabilityDataset({
    orgDatasetId: "org_1",
    files: allFiles({
      employee_skills: [{ ...employeeSkillRows[0], evidence_source: "Self Assessment" }],
      role_skill_requirements: [{ ...roleSkillRequirementRows[0], role_level: "Senior Manager" }],
    }),
    orgRows,
    orgEmployeeIdColumn: "employee_id",
  });

  const roleRecord = dataset.normalization.records.find((record) => record.field === "role_level");
  const evidenceRecord = dataset.normalization.records.find((record) => record.field === "evidence_source");

  assert.equal(dataset.normalization.needsReview, 2);
  assert.equal(roleRecord.status, "needs_review");
  assert.equal(dataset.states.asIs.roleSkillRequirements[0].role_level_normalized, "");
  ...
  assert.equal(approved.normalization.records.find((record) => record.id === roleRecord.id).status, "approved");
  assert.equal(approved.states.asIs.roleSkillRequirements[0].role_level_normalized, "Manager");
  ...
  assert.equal(ignored.normalization.records.find((record) => record.id === evidenceRecord.id).status, "ignored");
  assert.equal(ignored.states.asIs.employeeSkills[0].evidence_source, "Self Assessment");
  assert.equal(ignored.states.asIs.employeeSkills[0].evidence_source_normalized, "");
});
...
test("ignores extra descriptive columns during validation", () => {
  const dataset = createWorkCapabilityDataset({
    orgDatasetId: "org_1",
    files: allFiles({
      skill_library: [{ ...skillLibraryRows[0], description: "Optional imported description", aliases: "analytics, reporting" }],
      activity_assignments: [{ ...activityAssignmentsRows[0], notes: "Imported free-text note" }],
      employee_skills: [{ ...employeeSkillRows[0], notes: "Manager supplied context" }],
    }),
    orgRows,
    orgEmployeeIdColumn: "employee_id",
  });
  ...
  assert.equal(dataset.shared.skillLibrary[0].description, "Optional imported description");
  assert.equal(dataset.states.asIs.activityAssignments[0].notes, "Imported free-text note");
});

test("rejects missing employee, activity, and skill references", () => {
  const files = allFiles({
    activity_assignments: [{ ...activityAssignmentsRows[0], employee_id: "E999", activity_id: "ACT999" }],
    employee_skills: [{ ...employeeSkillRows[0], employee_id: "E888", skill_id: "SK999" }],
    activity_skill_requirements: [{ ...activitySkillRequirementRows[0], activity_id: "ACT998", skill_id: "SK998" }],
    role_skill_requirements: [{ ...roleSkillRequirementRows[0], skill_id: "SK997" }],
  });
...
test("builds activity portfolio metrics using allocation percentage", () => {
  ...
  const reporting = portfolio.activities.find((activity) => activity.activityId === "ACT001");
  const dashboards = portfolio.activities.find((activity) => activity.activityId === "ACT002");
...
test("builds taxonomy as domain category process activity hierarchy", () => {
  ...
  const opsActivity = portfolio.activities.find((activity) => activity.activityId === "ACT001");
  ...
  const domainNode = portfolio.tree.find((node) => node.kind === "domain" && node.domain === "Ops");
  ...
  const categoryNode = portfolio.tree.find((node) => node.kind === "category" && node.domain === "Ops" && node.category === "Reporting");
  assert.ok(categoryNode);
  assert.equal(categoryNode.processCount, 1);
  assert.equal(categoryNode.activityCount, 2);

  const processNode = portfolio.tree.find((node) => node.kind === "process" && node.domain === "Ops" && node.category === "Reporting" && node.processArea === "Operations");
  assert.ok(processNode);
  assert.equal(processNode.activityCount, 2);

  const graph = buildWorkCapabilityTaxonomyGraph(dataset, { selectedNodeId: "taxonomy-root" });
  const domainGraphNode = graph.nodes.find((node) => node.kind === "domain" && node.label === "Ops");
  const categoryGraphNode = graph.nodes.find((node) => node.kind === "category" && node.label === "Reporting" && node.parentId === domainGraphNode.id);
  const processGraphNode = graph.nodes.find((node) => node.kind === "process" && node.label === "Operations" && node.parentId === categoryGraphNode.id);
...
test("scopes category and process node ids by their full taxonomy path", () => {
  ...
  assert.ok(portfolio.tree.some((node) => node.id === "category::Ops::Reporting"));
  assert.ok(portfolio.tree.some((node) => node.id === "category::Transition::Reporting"));
  assert.ok(portfolio.tree.some((node) => node.id === "process::Ops::Reporting::Operations"));
  assert.ok(portfolio.tree.some((node) => node.id === "process::Transition::Reporting::Transition Management"));

  const graph = buildWorkCapabilityTaxonomyGraph(dataset, {});
  const reportingNodes = graph.nodes.filter((node) => node.kind === "category" && node.label === "Reporting");
  assert.equal(reportingNodes.length, 2);
  assert.deepEqual(
    reportingNodes.map((node) => node.id).sort(),
    ["category::Ops::Reporting", "category::Transition::Reporting"],
  );
  assert.equal(graph.nodes.some((node) => node.id === "category::Reporting"), false);
  assert.equal(graph.edges.filter((edge) => edge.target === "category::Ops::Reporting").length, 1);
  assert.equal(graph.edges.filter((edge) => edge.target === "category::Transition::Reporting").length, 1);
});
...
test("uses taxonomy fallbacks for blank department, category, and process values", () => {
  ...
  const reporting = portfolio.activities.find((activity) => activity.activityId === "ACT010");
  const general = portfolio.activities.find((activity) => activity.activityId === "ACT011");
...
test("activity portfolio excludes archived activities by default", () => {
  ...
  assert.equal(defaultPortfolio.activities.some((activity) => activity.activityId === "ACT001"), false);
  assert.equal(defaultPortfolio.kpis.totalActivities, 2);
  assert.equal(withArchived.activities.some((activity) => activity.activityId === "ACT001"), true);
...
test("classifies activity skill risk from assigned employee coverage", () => {
  const dataset = createWorkCapabilityDataset({
    orgDatasetId: "org_1",
    files: allFiles({
      activity_library: activityLibraryRows.map((row) => row.activity_id === "ACT003" ? { ...row, criticality: "Medium" } : row),
      activity_assignments: [
        activityAssignmentsRows[0],
        activityAssignmentsRows[1],
        { ...activityAssignmentsRows[0], assignment_id: "ASN003", activity_id: "ACT003", employee_id: "E1" },
      ],
      employee_skills: [
        { ...employeeSkillRows[0], employee_id: "E1", skill_id: "SK001", current_level: 4 },
        { ...employeeSkillRows[0], employee_skill_id: "ES002", employee_id: "E2", skill_id: "SK001", current_level: 2 },
      ],
      activity_skill_requirements: [
        { ...activitySkillRequirementRows[0], activity_id: "ACT001", required_level: 4 },
        { ...activitySkillRequirementRows[1], activity_id: "ACT002", required_level: 4 },
        { ...activitySkillRequirementRows[0], activity_skill_requirement_id: "ASR003", activity_id: "ACT003", required_level: 5 },
      ],
    }),
    orgRows,
    orgEmployeeIdColumn: "employee_id",
  ...
  assert.equal(portfolio.activities.find((activity) => activity.activityId === "ACT001").skillRisk, "Low");
  assert.equal(portfolio.activities.find((activity) => activity.activityId === "ACT002").skillRisk, "High");
  assert.equal(portfolio.activities.find((activity) => activity.activityId === "ACT003").skillRisk, "Medium");
  ...
  const unknownPortfolio = buildWorkCapabilityActivityPortfolio({
    dataset: { ...unknownBase, shared: { ...unknownBase.shared, activitySkillRequirements: [] } },
    orgRows,
    orgEmployeeIdColumn: "employee_id",
  });
  assert.equal(unknownPortfolio.activities.find((activity) => activity.activityId === "ACT001").skillRisk, "Unknown");
});
...
test("renames process and category taxonomy labels in canonical activity library", () => {
  ...
  assert.equal(renamedProcess.shared.activityLibrary.filter((row) => row.process_area === "Service Operations").length, 2);
  assert.equal(renamedProcess.shared.activityLibrary.find((row) => row.activity_id === "ACT003").process_area, "Transition Management");
  assert.equal(renamedProcess.revision, dataset.revision + 1);
  assert.equal(renamedProcess.taxonomyChangeLog.at(-1).action, "rename_process");
  ...
  assert.equal(renamedCategory.shared.activityLibrary.filter((row) => row.activity_category === "Performance Reporting").length, 2);
  assert.equal(renamedCategory.taxonomyChangeLog.at(-1).affected.activityRows, 2);
});
...
test("moves an activity category to another process as a single taxonomy mutation", () => {
  ...
  const movedRows = moved.shared.activityLibrary.filter((row) => row.activity_category === "Reporting");
  assert.equal(movedRows.length, 2);
  assert.equal(movedRows.every((row) => row.process_area === "Transition Management"), true);
...
test("moves and edits one activity without changing its stable activity id", () => {
  ...
  const activity = edited.shared.activityLibrary.find((row) => row.activity_id === "ACT001");
  ...
  assert.equal(edited.states.asIs.activityAssignments.find((row) => row.assignment_id === "ASN001").activity_id, "ACT001");
});
...
test("adds and deactivates activities while preserving rows for traceability", () => {
  ...
  assert.equal(added.shared.activityLibrary.some((row) => row.activity_id === "ACT999"), true);
  assert.equal(added.states.asIs.activityAssignments.some((row) => row.activity_id === "ACT999"), false);
  ...
  assert.equal(deactivated.shared.activityLibrary.find((row) => row.activity_id === "ACT999").active_flag, false);
  assert.equal(deactivated.taxonomyChangeLog.at(-1).action, "deactivate_activity");
});
...
test("merges activities by repointing assignments and skill requirements then deactivating sources", () => {
  ...
  assert.equal(merged.states.asIs.activityAssignments.find((row) => row.assignment_id === "ASN002").activity_id, "ACT001");
  assert.equal(merged.states.asIs.activityAssignments.some((row) => row.activity_id === "ACT002"), false);
  assert.equal(merged.shared.activitySkillRequirements.some((row) => row.activity_id === "ACT002"), false);
  assert.equal(merged.shared.activityLibrary.find((row) => row.activity_id === "ACT002").active_flag, false);
  assert.equal(merged.taxonomyChangeLog.at(-1).affected.assignmentRows, 1);
  assert.equal(merged.taxonomyChangeLog.at(-1).affected.skillRequirementRows, 1);
});
...
test("merge dedupes assignments and skill requirements after repointing", () => {
  const dataset = createWorkCapabilityDataset({
    ...
    files: allFiles({
      activity_assignments: [
        ...
        { ...activityAssignmentsRows[0], assignment_id: "ASN002", activity_id: "ACT002", time_allocation_pct: 15, notes: "second" },
      ],
      activity_skill_requirements: [
        { ...activitySkillRequirementRows[0], activity_id: "ACT001", required_level: 3, criticality: "Medium", weight: 0.6 },
        { ...activitySkillRequirementRows[1], activity_id: "ACT002", required_level: 4, criticality: "High", weight: 0.8 },
      ],
    }),
    orgRows,
    orgEmployeeIdColumn: "employee_id",
  ...
  const assignments = merged.states.asIs.activityAssignments.filter((row) => row.activity_id === "ACT001");
  ...
  const requirements = merged.shared.activitySkillRequirements.filter((row) => row.activity_id === "ACT001" && row.skill_id === "SK001");
  ...
  assert.equal(merged.shared.activityLibrary.find((row) => row.activity_id === "ACT002").merged_into_activity_id, "ACT001");
});
...
test("merge impact previews moved rows, deduped skill requirements, cost, and FTE", () => {
  ...
  const portfolio = buildWorkCapabilityActivityPortfolio({ dataset, orgRows, orgEmployeeIdColumn: "employee_id" });
...
test("archive impact previews people, cost, FTE, skills, and accountability links", () => {
  ...
  const impact = getWorkCapabilityArchiveImpact({ dataset, portfolio, activityIds: ["ACT002"] });
...
test("taxonomy cleanup suggestions flag duplicates, single-child chains, unassigned work, skill gaps, owner gaps, and archives", () => {
  const dataset = createWorkCapabilityDataset({
    ...
    files: allFiles({
      activity_library: [
        activityLibraryRows[0],
        { ...activityLibraryRows[1], activity_name: "Run reporting", active_flag: true },
        activityLibraryRows[2],
        { ...activityLibraryRows[2], activity_id: "ACT004", activity_name: "Archived transition work", active_flag: false },
      ],
  ...
  const portfolio = buildWorkCapabilityActivityPortfolio({ dataset, orgRows, orgEmployeeIdColumn: "employee_id", includeArchived: true });

  const suggestions = buildWorkCapabilityTaxonomyCleanupSuggestions({ dataset, portfolio });
  ...
  assert.ok(suggestions.find((suggestion) => suggestion.type === "possible_duplicate").actions.includes("Review"));
  assert.ok(suggestions.find((suggestion) => suggestion.type === "possible_duplicate").actions.includes("Merge"));
});
...
test("builds a visual taxonomy graph with branch-level activity expansion", () => {
  ...
  assert.ok(overview.nodes.some((node) => node.kind === "root" && node.id === "taxonomy-root"));
  assert.ok(overview.nodes.some((node) => node.kind === "domain" && node.label === "Transition"));
  assert.ok(overview.nodes.some((node) => node.kind === "category" && node.label === "Reporting"));
  assert.ok(overview.nodes.some((node) => node.kind === "process" && node.label === "Operations"));
  assert.equal(overview.nodes.some((node) => node.kind === "activity"), false);
  assert.ok(overview.edges.some((edge) => edge.source === "taxonomy-root" && edge.target.startsWith("domain::")));

  const expanded = buildWorkCapabilityTaxonomyGraph(dataset, {
    selectedNodeId: "process::Ops::Reporting::Operations",
  });

  const activityNode = expanded.nodes.find((node) => node.kind === "activity" && node.activityId === "ACT002");
...
test("Work & Capability upload is a multi-step modal and hides issues until import", () => {
  ...
  assert.match(page, /\{hasImportAttempted && \(/);
  assert.match(page, /\{hasImportAttempted && normalization && normalization\.totalGroups > 0 && \(/);
